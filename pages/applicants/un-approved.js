import { Form, ButtonGroup, Table, Tab, Nav } from "react-bootstrap";
...
export async function getServerSideProps(context) {
  ...
  const [
    ...
    `/applicants/unapproved?limit=${limit}&${querystring.stringify(
      context.query
    )}`,
    "GET"
  );
...
export default function Index({
  ...
  const handleStatusChange = async (active, id) => {
    try {
      ...
      if (response.status === 200) {
        toast.info(
          `Applicant ${active ? "approved" : "unapproved"} successfully`
        );
  ...
  return (
    <Main
      title={`Applicants || UnApproved (${total})`}
